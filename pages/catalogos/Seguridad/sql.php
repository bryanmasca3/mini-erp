  ...
  if(isset($_SESSION["usr_ID"])){
    ...
    break;
      ...
      case "01_delete_sintomatologia":
        ...
        echo json_encode($rpta);
      break;   
      case "01_delete_checklist_camio_cis":
    ...
    //respuesta OT
      ...
      case "01_get_pernocte_grid":        
        ...
        $qry2 = $db->query("SELECT count(*) cantidad, H.id_pernocte as id_pernoctes, T.* FROM `tb_trabajador` as T INNER JOIN `tb_horario_pernoecte` as H ON T.id = H.id_trabajador INNER JOIN `tb_pernocte` as P ON P.id = H.id_pernocte where P.id='".+$data->value."' GROUP by id_trabajador");
        $totreg2 = $db->num_rows($qry2);
      ...
      case "03_pdf_pernocte":        
        ...
        $qry2 = $db->query("SELECT T.id as id_trabajadores,T.nombres as nombres,T.apellidos as apellidos, T.cargo as cargo,T.dni as dni,H.hora_inicio as hora_inicio,H.hora_fin as hora_fin,P.* FROM `tb_trabajador` as T INNER JOIN `tb_horario_pernoecte` as H ON T.id = H.id_trabajador INNER JOIN `tb_pernocte` as P ON P.id = H.id_pernocte where P.id='".+$data->id."'");
        $totreg2 = $db->num_rows($qry2);
    ...
    //respuesta OT
      ...
      case "01_grid_checkList":
        $tabla = array();    
        $qry = $db->query("SELECT CK.*,C.nombres as nombres,C.apellidos as apellidos,C.dni as dni,'camioneta' AS tipo FROM `tb_checklist_camioneta` as CK INNER JOIN `tb_conductor` as C ON CK.id_conductor=C.id;");
        $totreg = $db->num_rows($qry);
        if ($totreg>0) {
          for($xx=0; $xx<$totreg; $xx++){
            $rs = $db->fetch_array($qry);
    ...
    //respuesta OT
      ...
      echo json_encode($rpta);
      break;
      case "03_pdf_checkList_camio_cister":
        $tabla = array(); 
        $tabla1 = array(); 
